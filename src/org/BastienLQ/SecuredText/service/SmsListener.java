...
import org.BastienLQ.SecuredText.jobs.SmsReceiveJob;
...
//  }
  ...
  private boolean isRelevant(Context context, Intent intent) {
    ...
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT &&
        SecuredTextPreferences.isInterceptAllSmsEnabled(context))
    {
      return true;
    }
  ...
  private boolean isChallenge(Context context, Intent intent) {
    ...
    if (messageBody.matches(".*Your SecuredText verification code: [0-9]{3,4}-[0-9]{3,4}") &&
        SecuredTextPreferences.isVerifying(context))
    {
      return true;
    }
