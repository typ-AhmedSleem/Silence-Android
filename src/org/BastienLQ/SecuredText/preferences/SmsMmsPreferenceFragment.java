package org.BastienLQ.SecuredText.preferences;
...
import org.BastienLQ.SecuredText.ApplicationPreferencesActivity;
import org.BastienLQ.SecuredText.R;
import org.BastienLQ.SecuredText.components.OutgoingSmsPreference;
...
public class SmsMmsPreferenceFragment extends PreferenceFragment {
  ...
  private void initializePlatformSpecificOptions() {
    ...
    Preference         allSmsPreference         = findPreference(SecuredTextPreferences.ALL_SMS_PREF);
    Preference         allMmsPreference         = findPreference(SecuredTextPreferences.ALL_MMS_PREF);
  ...
  private String buildOutgoingSmsDescription() {
    ...
    final boolean       dataFallback    = SecuredTextPreferences.isFallbackSmsAllowed(getActivity());
    final boolean       dataFallbackAsk = SecuredTextPreferences.isFallbackSmsAskRequired(getActivity());
    final boolean       mmsFallback     = SecuredTextPreferences.isFallbackMmsEnabled(getActivity());
    final boolean       nonData         = SecuredTextPreferences.isDirectSmsAllowed(getActivity());
  ...
  private static CharSequence getIncomingSmsSummary(Context context) {
    ...
    boolean preKitkatSMS  = SecuredTextPreferences.isInterceptAllSmsEnabled(context);
    boolean preKitkatMMS  = SecuredTextPreferences.isInterceptAllMmsEnabled(context);
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
  ...
  private static CharSequence getOutgoingSmsSummary(Context context) {
    ...
    if (SecuredTextPreferences.isFallbackSmsAllowed(context) && SecuredTextPreferences.isDirectSmsAllowed(context)) {
      outgoingSmsSummary = onResId;
    } else if (SecuredTextPreferences.isFallbackSmsAllowed(context) ^ SecuredTextPreferences.isDirectSmsAllowed(context)) {
