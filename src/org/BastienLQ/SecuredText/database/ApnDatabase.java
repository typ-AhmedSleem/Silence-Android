...
import org.BastienLQ.SecuredText.mms.MmsConnection.Apn;
...
public class ApnDatabase {
  ...
  private Apn getCustomApnParameters() {
      String mmsc = SecuredTextPreferences.getMmscUrl(context).trim();

      if (!TextUtils.isEmpty(mmsc) && !mmsc.startsWith("http"))
        mmsc = "http://" + mmsc;

      String proxy = SecuredTextPreferences.getMmscProxy(context);
      String port  = SecuredTextPreferences.getMmscProxyPort(context);
      String user  = SecuredTextPreferences.getMmscUsername(context);
      String pass  = SecuredTextPreferences.getMmscPassword(context);
  ...
  public Optional<Apn> getMmsConnectionParameters(String mccmnc, String apn) {
    ...
    Apn result     = new Apn(customApn, defaultApn,
                             SecuredTextPreferences.getUseCustomMmsc(context),
                             SecuredTextPreferences.getUseCustomMmscProxy(context),
                             SecuredTextPreferences.getUseCustomMmscProxyPort(context),
                             SecuredTextPreferences.getUseCustomMmscUsername(context),
                             SecuredTextPreferences.getUseCustomMmscPassword(context));
