...
public class DirectoryHelper {
  ...
  public static void refreshDirectory(final Context context) throws IOException {
    refreshDirectory(context, SecuredTextCommunicationFactory.createManager(context));
  }

  public static void refreshDirectory(final Context context, final TextSecureAccountManager accountManager)
      throws IOException
  {
    refreshDirectory(context, accountManager, SecuredTextPreferences.getLocalNumber(context));
  }
  ...
  public static boolean isPushDestination(Context context, Recipients recipients) {
    try {
      ...
      return SecuredTextDirectory.getInstance(context).isActiveNumber(e164number);
  ...
  public static boolean isSmsFallbackAllowed(Context context, Recipients recipients) {
    try {
      ...
      return SecuredTextDirectory.getInstance(context).isSmsFallbackSupported(e164number);
    } catch (InvalidNumberException e) {
      Log.w(TAG, e);
      return false;
    }
