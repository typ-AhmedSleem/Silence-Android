        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            if (oldVersion < INTRODUCED_NEW_TYPES_VERSION) {
                ...
                db.execSQL("UPDATE sms SET body = substr(body, ?), type = type | ? WHERE body LIKE ?",
                        new String[]{String.valueOf(SYMMETRIC_ENCRYPT.length() + 1),
                                0x80000000L + "",
                                SYMMETRIC_ENCRYPT + "%"});

                db.execSQL("UPDATE sms SET body = substr(body, ?), type = type | ? WHERE body LIKE ?",
                        new String[]{String.valueOf(ASYMMETRIC_LOCAL_ENCRYPT.length() + 1),
                                0x40000000L + "",
                                ASYMMETRIC_LOCAL_ENCRYPT + "%"});

                db.execSQL("UPDATE sms SET body = substr(body, ?), type = type | ? WHERE body LIKE ?",
                        new String[]{String.valueOf(ASYMMETRIC_ENCRYPT.length() + 1),
                                (0x800000L | 0x20000000L) + "",
                                ASYMMETRIC_ENCRYPT + "%"});

                db.execSQL("UPDATE sms SET body = substr(body, ?), type = type | ? WHERE body LIKE ?",
                        new String[]{String.valueOf(KEY_EXCHANGE.length() + 1),
                                0x8000L + "",
                                KEY_EXCHANGE + "%"});

                db.execSQL("UPDATE sms SET body = substr(body, ?), type = type | ? WHERE body LIKE ?",
                        new String[]{String.valueOf(PROCESSED_KEY_EXCHANGE.length() + 1),
                                (0x8000L | 0x2000L) + "",
                                PROCESSED_KEY_EXCHANGE + "%"});

                db.execSQL("UPDATE sms SET body = substr(body, ?), type = type | ? WHERE body LIKE ?",
                        new String[]{String.valueOf(STALE_KEY_EXCHANGE.length() + 1),
                                (0x8000L | 0x4000L) + "",
                                STALE_KEY_EXCHANGE + "%"});
            ...
            if (oldVersion < INTRODUCED_MMS_FROM_VERSION) {
                ...
                while (cursor != null && cursor.moveToNext()) {
                    ...
                    if (!TextUtils.isEmpty(address)) {
                        db.execSQL("UPDATE mms SET address = ? WHERE _id = ?", new String[]{address, String.valueOf(mmsId)});
                    }
                }
                ...
                for (long thread : threads) {
                    ...
                    try {
                        cursor = db.rawQuery("SELECT DISTINCT date AS date_received, status " +
                                ...
                                "ORDER BY date_received DESC LIMIT 1", new String[]{String.valueOf(thread)});

                        if (cursor != null && cursor.moveToNext()) {
                            int status = cursor.getInt(cursor.getColumnIndexOrThrow("status"));

                            db.execSQL("UPDATE thread SET status = ? WHERE _id = ?",
                                    new String[]{String.valueOf(status), String.valueOf(thread)});
